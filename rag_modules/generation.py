...
class GenerationModule:
    ...
    def __init__(self, model_name: str = "glm-4-flash", temperature: float = 0, max_tokens: int = 2048):
...
改写后：""",
    ...
    def generate_answer(self, query: str, context_docs: List[Document]) -> str:
        ...
        prompt = ChatPromptTemplate.from_template("""你是企业规章制度问答助手。你的唯一任务是从参考文档中找到答案并直接引用。

严格规则（违反则失败）：
- 只允许使用参考文档中原文出现的信息
- 禁止任何推断、补充、扩展或建议
- 禁止使用文档中未出现的数字、日期、流程
- 如果文档中找不到答案，只回答：根据现有文档无法回答此问题，请联系相关部门。

回答格式：直接给出答案，不加任何额外解释。
